...
class ros_skid_steering_pure_hdg_controller:
    ...
    def __init__(
        ...
        self.pub_telem_targ_hdg = rospy.Publisher(self.telem_topic_prefix+'targ_hdg', self.telem_type, queue_size=None)
        self.msg_telem_targ_hdg = self.telem_type()
        ...
        self.pub_telem_hdg_rate = rospy.Publisher(self.telem_topic_prefix+'hdg_rate', self.telem_type, queue_size=None)
        self.msg_telem_hdg_rate = self.telem_type()

    def get_desired_vel_hdg( self, time_val ):
        print(self.commands)
        ...
        for idx in range( len( self.commands.index )-1) :
            if self.commands['time'].iloc[idx] <= time_val and self.commands['time'].iloc[idx+1] > time_val:
                ...
                return (self.commands['heading'].iloc[idx], self.commands['velocity'].iloc[idx])
        
        return (self.rov_theta_current,0)
    ...
    def callback_rover_gps( self, data ):
        if self.rov_theta_current != None:
            ...
            (targ_hdg, targ_vel ) = self.get_desired_vel_hdg( t )

            # Produce the control feedback
            #((rvel, lvel), lateness, halt ) = self.vc.controller_step( t, rov_x, rov_y, rov_theta )
            hdg_rate = self.hc.step( targ_hdg, rov_theta, t) 
            (rvel, lvel) = self.tf.transform(hdg_rate, targ_vel)
            ...
            self.msg_telem_targ_hdg.data = targ_hdg 
            self.msg_telem_targ_vel.data = targ_vel 
            self.msg_telem_hdg_rate.data = hdg_rate 

            self.pub_lt.publish( self.msg_lt )
            self.pub_rt.publish( self.msg_rt )
            self.pub_telem_targ_hdg.publish( self.msg_telem_targ_hdg )
            self.pub_telem_targ_vel.publish( self.msg_telem_targ_vel )
            self.pub_telem_hdg_rate.publish( self.msg_telem_hdg_rate )
...
if __name__ == '__main__' :
    ...
    df = pd.DataFrame(
        {
            'time':[0,2,22,42,62,82],
            'velocity':[0,4.0,4.0,4.0,4.0,4.0],
            'heading':[np.pi,np.pi,np.pi/2,0,3*np.pi/2, np.pi]
        }
    )
